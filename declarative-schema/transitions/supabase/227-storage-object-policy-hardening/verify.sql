  ...
  and (
    select count(*) = 1
      and bool_and(
        ...
        and pg_get_expr(polwithcheck, polrelid) ilike '%storage.foldername%'
        and pg_get_expr(polwithcheck, polrelid) ilike '%auth.jwt%'
        and pg_get_expr(polwithcheck, polrelid) ilike '%sub%'
      )
    from pg_policy
    where polrelid = 'storage.objects'::regclass
      and polname = 'transition_storage_insert_227'
