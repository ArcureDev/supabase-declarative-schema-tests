# Case: 506-routine-drop

- Scenario pack: `Functions, procedures, aggregates, and triggers catalogue scenarios` / `routine-drop`
- Catalogue atoms: `PG-CAT-RTN-02::routine.drop`

## Baseline state A

```sql
-- Covers PG-CAT-RTN-02::routine.drop. Keep public.transition_anchor identity stable.
create table public.transition_anchor (
  id bigint primary key,
  payload text not null
);

create table public.catalogue_routine_drop (
...
- Command: `npx supabase db schema declarative sync --apply --name 506_routine_drop_baseline --debug`
- Result: **SKIPPED**
- Duration: `0.0s`
...
```text
The declarative baseline failed, so the apply routine.drop with a native in-place operation transition was skipped.
```
<!-- declarative-schema-command-result case="506-routine-drop" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.catalogue_routine_drop
  ADD CONSTRAINT catalogue_routine_drop_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_routine_drop TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_routine_drop TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_routine_drop TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\506-routine-drop-legacy\supabase\migrations\20260816012250_506_routine_drop_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\506-routine-drop-legacy\supabase\.temp\pgdelta\debug\20260816-012250-apply-error
...
The declarative baseline failed, so the apply routine.drop with a native in-place operation transition was skipped.
```
<!-- declarative-schema-command-result case="506-routine-drop" engine="legacy" command="sync" status="ERROR" -->
