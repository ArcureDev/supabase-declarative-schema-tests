...
- Scenario pack: `Constraints, indexes, statistics, and rules catalogue scenarios` / `rule-drop`
- Catalogue atoms: `PG-CAT-CIX-09::rule.drop`

## Baseline state A

```sql
-- Covers PG-CAT-CIX-09::rule.drop. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for rule.drop must be refused, not silently omitted.
```
...
### `20260815214119_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\433-rule-drop\supabase\migrations\20260815214119_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="433-rule-drop" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="433-rule-drop" engine="next" command="sync-verification" status="OK" -->
...
### `20260815214212_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\433-rule-drop-legacy\supabase\migrations\20260815214212_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="433-rule-drop" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="433-rule-drop" engine="legacy" command="sync-verification" status="OK" -->
