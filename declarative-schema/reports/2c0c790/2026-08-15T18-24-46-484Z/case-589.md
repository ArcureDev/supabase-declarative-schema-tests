# Case: 589-seclabel-provider

- Scenario pack: `Extensions, FDWs, and external boundaries catalogue scenarios` / `seclabel-provider`
- Catalogue atoms: `PG-CAT-EXT-06::seclabel.provider`

## Baseline state A

```sql
-- Covers PG-CAT-EXT-06::seclabel.provider. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for seclabel.provider must be refused, not silently omitted.
```
...
### `20260816051535_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\589-seclabel-provider\supabase\migrations\20260816051535_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="589-seclabel-provider" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="589-seclabel-provider" engine="next" command="sync-verification" status="OK" -->
...
### `20260816051627_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\589-seclabel-provider-legacy\supabase\migrations\20260816051627_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="589-seclabel-provider" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="589-seclabel-provider" engine="legacy" command="sync-verification" status="OK" -->
