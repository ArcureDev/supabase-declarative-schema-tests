# Case: 535-ownership-reassign

- Scenario pack: `Roles, ownership, grants, and row-level security catalogue scenarios` / `ownership-reassign`
- Catalogue atoms: `PG-CAT-ROL-02::ownership.reassign`

## Baseline state A

```sql
-- Covers PG-CAT-ROL-02::ownership.reassign. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for ownership.reassign must be refused, not silently omitted.
```
...
### `20260816030213_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\535-ownership-reassign\supabase\migrations\20260816030213_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="535-ownership-reassign" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="535-ownership-reassign" engine="next" command="sync-verification" status="OK" -->
...
### `20260816030306_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\535-ownership-reassign-legacy\supabase\migrations\20260816030306_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="535-ownership-reassign" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="535-ownership-reassign" engine="legacy" command="sync-verification" status="OK" -->
