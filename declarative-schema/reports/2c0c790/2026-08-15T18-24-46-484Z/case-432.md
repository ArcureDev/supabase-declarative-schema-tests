# Case: 432-rule-replace

- Scenario pack: `Constraints, indexes, statistics, and rules catalogue scenarios` / `rule-replace`
- Catalogue atoms: `PG-CAT-CIX-09::rule.replace`

## Baseline state A

```sql
-- Covers PG-CAT-CIX-09::rule.replace. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for rule.replace must be refused, not silently omitted.
```
...
### `20260815213940_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\432-rule-replace\supabase\migrations\20260815213940_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="432-rule-replace" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="432-rule-replace" engine="next" command="sync-verification" status="OK" -->
...
### `20260815214032_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\432-rule-replace-legacy\supabase\migrations\20260815214032_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="432-rule-replace" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="432-rule-replace" engine="legacy" command="sync-verification" status="OK" -->
