# Case: 578-extension-schema-move

- Scenario pack: `Extensions, FDWs, and external boundaries catalogue scenarios` / `extension-schema-move`
- Catalogue atoms: `PG-CAT-EXT-01::extension.schema-move`
...
- Command: `npx supabase db schema declarative sync --apply --name 578_extension_schema_move_baseline --debug`
- Result: **SKIPPED**
- Duration: `0.0s`
...
```text
The declarative baseline failed, so the apply extension.schema-move with a native in-place operation transition was skipped.
```
<!-- declarative-schema-command-result case="578-extension-schema-move" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.catalogue_extension_schema_move
  ADD CONSTRAINT catalogue_extension_schema_move_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_extension_schema_move TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_extension_schema_move TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_extension_schema_move TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\578-extension-schema-move-legacy\supabase\migrations\20260816050146_578_extension_schema_move_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\578-extension-schema-move-legacy\supabase\.temp\pgdelta\debug\20260816-050146-apply-error
...
The declarative baseline failed, so the apply extension.schema-move with a native in-place operation transition was skipped.
```
<!-- declarative-schema-command-result case="578-extension-schema-move" engine="legacy" command="sync" status="ERROR" -->
