# Case: 319-drop-column-at-populated

- Scenario pack: `Schemas, tables, columns, and sequences catalogue scenarios` / `drop-column-at-populated`
- Catalogue atoms: `PG-CAT-STC-03::drop.column@populated`
...
create table public.catalogue_items (
  id bigint primary key, label text, leftover text
);
```
...
### `20260815185144_destructive_change_warning_baseline.sql`
...
grant maintain, references, trigger, truncate on table "public"."catalogue_items" to "anon", "authenticated";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "public"."catalogue_items" to "postgres";

grant maintain, references, trigger, truncate on table "public"."catalogue_items" to "service_role";
...
- Raw sync result: **OK**
- Assertion: **WARNING**
- The command completed without satisfying the destructive-column-drop safety contract. No explicit destructive-change or data-loss diagnostic was present. The output identified public.catalogue_items. The output identified column leftover.

## Generated transition migration files

### `20260815185158_declarative_sync.sql`

```sql
alter table "public"."catalogue_items"
  drop column "leftover";
```
...
Applying migration 20260815185144_destructive_change_warning_baseline.sql...
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\319-drop-column-at-populated\supabase\migrations\20260815185158_declarative_sync.sql
Found destructive changes in schema diff. Please double check if these are expected:
ALTER TABLE "public"."catalogue_items" DROP COLUMN "leftover"
...
alter table "public"."catalogue_items"
  drop column "leftover";
```
<!-- declarative-schema-command-result case="319-drop-column-at-populated" engine="next" command="sync" status="WARNING" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="319-drop-column-at-populated" engine="next" command="sync-verification" status="OK" -->
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_items TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_items TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_items TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\319-drop-column-at-populated-legacy\supabase\migrations\20260815185251_destructive_change_warning_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\319-drop-column-at-populated-legacy\supabase\.temp\pgdelta\debug\20260815-185251-apply-error
...
```text
The declarative baseline failed, so the destructive column drop transition was skipped.
```
<!-- declarative-schema-command-result case="319-drop-column-at-populated" engine="legacy" command="sync" status="ERROR" -->
