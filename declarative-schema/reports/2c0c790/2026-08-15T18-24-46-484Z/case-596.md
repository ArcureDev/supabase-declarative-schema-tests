# Case: 596-boundary-sequence-value

- Scenario pack: `Explicit PostgreSQL boundaries catalogue scenarios` / `boundary-sequence-value`
- Catalogue atoms: `PG-CAT-BND-03::boundary.sequence-value`

## Baseline state A

```sql
-- Covers PG-CAT-BND-03::boundary.sequence-value. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for boundary.sequence-value must be refused, not silently omitted.
```
...
### `20260816052716_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\596-boundary-sequence-value\supabase\migrations\20260816052716_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="596-boundary-sequence-value" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="596-boundary-sequence-value" engine="next" command="sync-verification" status="OK" -->
...
### `20260816052809_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\596-boundary-sequence-value-legacy\supabase\migrations\20260816052809_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="596-boundary-sequence-value" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="596-boundary-sequence-value" engine="legacy" command="sync-verification" status="OK" -->
