# Case: 539-grant-database

- Scenario pack: `Roles, ownership, grants, and row-level security catalogue scenarios` / `grant-database`
- Catalogue atoms: `PG-CAT-ROL-03::grant.database`

## Baseline state A

```sql
-- Covers PG-CAT-ROL-03::grant.database. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for grant.database must be refused, not silently omitted.
```
...
### `20260816030741_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\539-grant-database\supabase\migrations\20260816030741_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="539-grant-database" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="539-grant-database" engine="next" command="sync-verification" status="OK" -->
...
### `20260816030834_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\539-grant-database-legacy\supabase\migrations\20260816030834_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="539-grant-database" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="539-grant-database" engine="legacy" command="sync-verification" status="OK" -->
