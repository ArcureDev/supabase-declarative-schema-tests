# Case: 416-index-covering

- Scenario pack: `Constraints, indexes, statistics, and rules catalogue scenarios` / `index-covering`
- Catalogue atoms: `PG-CAT-CIX-06::index.covering`

## Baseline state A

```sql
-- Covers PG-CAT-CIX-06::index.covering. Keep public.transition_anchor identity stable.
create table public.transition_anchor (
  id bigint primary key,
  payload text not null
);

create table public.catalogue_index_covering (
...
- Command: `npx supabase db schema declarative sync --apply --name 416_index_covering_baseline --debug`
- Result: **SKIPPED**
- Duration: `0.0s`
...
```text
The declarative baseline failed, so the apply index.covering with a native in-place operation transition was skipped.
```
<!-- declarative-schema-command-result case="416-index-covering" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.catalogue_index_covering
  ADD CONSTRAINT catalogue_index_covering_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_index_covering TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_index_covering TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.catalogue_index_covering TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\416-index-covering-legacy\supabase\migrations\20260815211737_416_index_covering_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\416-index-covering-legacy\supabase\.temp\pgdelta\debug\20260815-211738-apply-error
...
The declarative baseline failed, so the apply index.covering with a native in-place operation transition was skipped.
```
<!-- declarative-schema-command-result case="416-index-covering" engine="legacy" command="sync" status="ERROR" -->
