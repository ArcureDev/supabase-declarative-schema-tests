# Case: 595-boundary-temp-objects

- Scenario pack: `Explicit PostgreSQL boundaries catalogue scenarios` / `boundary-temp-objects`
- Catalogue atoms: `PG-CAT-BND-03::boundary.temp-objects`

## Baseline state A

```sql
-- Covers PG-CAT-BND-03::boundary.temp-objects. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for boundary.temp-objects must be refused, not silently omitted.
```
...
### `20260816052535_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\595-boundary-temp-objects\supabase\migrations\20260816052535_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="595-boundary-temp-objects" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="595-boundary-temp-objects" engine="next" command="sync-verification" status="OK" -->
...
### `20260816052628_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-DHzNgi\595-boundary-temp-objects-legacy\supabase\migrations\20260816052628_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="595-boundary-temp-objects" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="595-boundary-temp-objects" engine="legacy" command="sync-verification" status="OK" -->
