# Case: 260-broadcast-presence-policies
...
using (
  extension in ('broadcast', 'presence')
  and realtime.topic() like 'team:%'
  and split_part(realtime.topic(), ':', 2) = (select auth.jwt() ->> 'team_id')
);
...
- Command: `npx supabase db schema declarative sync --apply --name 260_broadcast_presence_policies_baseline --debug`
- Result: **ERROR**
- Duration: `59.2s`
- Exit code: `1`
...
```text
The declarative baseline failed, so the scope Realtime Broadcast and Presence policies to the caller team transition was skipped.
```
<!-- declarative-schema-command-result case="260-broadcast-presence-policies" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.realtime_policy_identity_260
  ADD CONSTRAINT realtime_policy_identity_260_pkey PRIMARY KEY (policy_name);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_policy_identity_260 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_policy_identity_260 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_policy_identity_260 TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\260-broadcast-presence-policies-legacy\supabase\migrations\20260817233052_260_broadcast_presence_policies_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\260-broadcast-presence-policies-legacy\supabase\.temp\pgdelta\debug\20260817-233052-apply-error
...
The declarative baseline failed, so the scope Realtime Broadcast and Presence policies to the caller team transition was skipped.
```
<!-- declarative-schema-command-result case="260-broadcast-presence-policies" engine="legacy" command="sync" status="ERROR" -->
