# Case: 221-text-search-mapping-transition
...
### `20260817194058_declarative_sync.sql`
...
│
▲  pg-delta does not manage these PostgreSQL object kinds: text search configuration. Changes to these objects are omitted from the declarative migration plan.
│
...
pg-delta next diagnostic: origin=declarativeLoad code=unmodeled_kind message=1 unmodeled "text search configuration" object not managed by this engine (e.g. transition_search_221) — v1 detects but does not model this kind; keep its DDL in _custom/ so re-exports preserve it and the shadow can elaborate dependents, and deliver it to targets via your migration channel
...
pg-delta next diagnostic: origin=declarativeDrift code=unmodeled_drift message=1 unmodeled "text search configuration" object exist in the desired state (shadow) but NOT on the target (public.transition_search_221) — this engine models no facts for this kind, so no planned statement can create them, and any planned statement that depends on one will fail on the target. Deliver them through the same migration channel that delivers your _custom/ SQL, then re-plan
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\221-text-search-mapping-transition\supabase\migrations\20260817194058_declarative_sync.sql
...
- Result: **OK**
- Duration: `59.1s`
<!-- declarative-schema-command-result case="221-text-search-mapping-transition" engine="next" command="sync" status="OK" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="221-text-search-mapping-transition" engine="next" command="sync-verification" status="OK" -->
