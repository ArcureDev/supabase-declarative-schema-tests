# Case: 226-auth-uid-policy-hardening
...
### `20260817195251_226_auth_uid_policy_hardening_baseline.sql`
...
create table "public"."auth_documents_226" (
  ...
  "archived" boolean not null default false,
  "body"     text    not null,
  constraint "auth_documents_226_pkey" primary key (id)
);

alter table "public"."auth_documents_226"
...
create policy "auth_owned_documents_226" on "public"."auth_documents_226"
  for all
  to "authenticated"
  using ((owner_id = ( select auth.uid() as uid)))
  with check ((owner_id = ( SELECT auth.uid() AS uid)));

grant maintain, references, trigger, truncate on table "public"."auth_documents_226" to "anon";

grant maintain, references, select, trigger, truncate on table "public"."auth_documents_226" to "authenticated";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "public"."auth_documents_226" to "postgres";

grant maintain, references, trigger, truncate on table "public"."auth_documents_226" to "service_role";
...
- Raw sync result: **OK**
- Assertion: **ERROR**
- The generated migration did not satisfy its declared SQL-shape contract. Missing operation(s): alter the Auth-facing policy, grant update access. Forbidden operation(s): do not weaken or recreate security.

## Generated transition migration files

### `20260817195331_declarative_sync.sql`

```sql
drop policy "auth_owned_documents_226" on "public"."auth_documents_226";

create policy "auth_owned_documents_226" on "public"."auth_documents_226"
  for all
  to "authenticated"
  using (((owner_id = ( select auth.uid() as uid)) AND (not archived)))
  with check (((owner_id = ( SELECT auth.uid() AS uid)) AND (NOT archived)));

revoke all on table "public"."auth_documents_226" from "authenticated";

grant maintain, references, select, trigger, truncate, update on table "public"."auth_documents_226" to "authenticated";
```
...
- Command: `npx supabase db schema declarative sync --apply --name 226_auth_uid_policy_hardening_baseline --debug`
- Result: **OK**
- Duration: `39.9s`
...
Applying migration 20260817195251_226_auth_uid_policy_hardening_baseline.sql...
Initialising schema...
+ ulimit -n
+ '[' -n '' ']'
+ export ERL_CRASH_DUMP=/tmp/erl_crash.dump
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:public.auth_documents_226_id_seq message=edge sequence:public.auth_documents_226_id_seq -[depends]-> schema:public references a fact not in the base
...
pg-delta next diagnostic: origin=declarativeTarget code=dangling_edge subject=sequence:public.auth_documents_226_id_seq message=edge sequence:public.auth_documents_226_id_seq -[depends]-> schema:public references a fact not in the base
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\226-auth-uid-policy-hardening\supabase\migrations\20260817195331_declarative_sync.sql
...
grant maintain, references, select, trigger, truncate, update on table "public"."auth_documents_226" to "authenticated";
```
<!-- declarative-schema-command-result case="226-auth-uid-policy-hardening" engine="next" command="sync" status="ERROR" -->
...
```
<!-- declarative-schema-command-result case="226-auth-uid-policy-hardening" engine="next" command="sync-verification" status="ERROR" -->
...
ALTER TABLE public.auth_documents_226
  ADD CONSTRAINT auth_documents_226_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.auth_documents_226 TO anon;

GRANT MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE ON public.auth_documents_226 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.auth_documents_226 TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\226-auth-uid-policy-hardening-legacy\supabase\migrations\20260817195438_226_auth_uid_policy_hardening_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\226-auth-uid-policy-hardening-legacy\supabase\.temp\pgdelta\debug\20260817-195438-apply-error
...
```text
The declarative baseline failed, so the preserve policy identity while hardening auth.uid checks and adding only update access transition was skipped.
```
<!-- declarative-schema-command-result case="226-auth-uid-policy-hardening" engine="legacy" command="sync" status="ERROR" -->
