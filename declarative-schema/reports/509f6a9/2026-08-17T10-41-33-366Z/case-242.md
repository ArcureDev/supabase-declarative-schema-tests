# Case: 242-background-processing-release
...
### `20260817214531_242_background_processing_release_baseline.sql`
...
create table "jobs"."queue" (
  ...
  constraint "queue_status_check" check ((status = ANY (ARRAY['queued'::text, 'working'::text, 'done'::text])))
);
...
create table "jobs"."schedules" (
  ...
  "command_name"    text    not null,
  "enabled"         boolean not null default true,
  constraint "schedules_pkey" primary key (name)
);
...
create or replace function jobs.enqueue (
  ...
  language plpgsql
  security definer
  set search_path to 'pg_catalog', 'jobs'
  AS $function$
declare
  queued_id bigint;
begin
...
create policy "queue_submit" on "jobs"."queue"
  for insert
  to "authenticated"
  with check (true);

grant execute on function "jobs"."enqueue"(text, jsonb) to "postgres";

grant create, usage on schema "jobs" to "postgres";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "jobs"."queue" to "postgres";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "jobs"."schedules" to "postgres";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "jobs"."secret_refs" to "postgres";
...
- Raw sync result: **OK**
- Assertion: **ERROR**
- The generated migration did not satisfy its declared SQL-shape contract. Missing operation(s): creates the skip-locked claim routine.

## Generated transition migration files

### `20260817214621_declarative_sync.sql`

```sql
set local check_function_bodies = off;

alter table "jobs"."queue"
  drop constraint "queue_status_check";
...
create table "jobs"."worker_outbox" (
  ...
  "event_kind" text   not null,
  "payload"    jsonb  not null,
  constraint "worker_outbox_pkey" primary key (id)
);
...
create or replace function jobs.dead_letter_notify()
  returns trigger
  language plpgsql
  set search_path to 'pg_catalog', 'jobs'
  AS $function$
...
alter table "jobs"."queue"
  add constraint "queue_status_check" check ((status = ANY (ARRAY['queued'::text, 'retry'::text, 'working'::text, 'done'::text])));

create index queue_claim_idx on jobs.queue using btree (queue_name, available_at, id)
  where (status = ANY (ARRAY['queued'::text, 'retry'::text]));
...
revoke all on function "jobs"."claim_next"(text, text) from public;

grant execute on function "jobs"."claim_next"(text, text) to "postgres", "service_role";

grant execute on function "jobs"."dead_letter_notify"() to "postgres";

revoke all on function "jobs"."enqueue"(text, jsonb) from public;

revoke all on function "jobs"."enqueue"(text, jsonb) from "authenticated";

grant execute on function "jobs"."enqueue"(text, jsonb) to "authenticated";

revoke all on schema "jobs" from "authenticated";

grant usage on schema "jobs" to "authenticated";

revoke all on schema "jobs" from "service_role";

grant usage on schema "jobs" to "service_role";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "jobs"."dead_letters" to "postgres";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "jobs"."worker_outbox" to "postgres";
```
...
- Command: `npx supabase db schema declarative sync --apply --name 242_background_processing_release_baseline --debug`
- Result: **OK**
- Duration: `59.3s`
...
Applying migration 20260817214531_242_background_processing_release_baseline.sql...
Initialising schema...
+ ulimit -n
+ '[' -n '' ']'
+ export ERL_CRASH_DUMP=/tmp/erl_crash.dump
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:jobs.worker_outbox_id_seq message=edge sequence:jobs.worker_outbox_id_seq -[depends]-> schema:jobs references a fact not in the base
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:jobs.queue_id_seq message=edge sequence:jobs.queue_id_seq -[depends]-> schema:jobs references a fact not in the base
...
pg-delta next diagnostic: origin=declarativeTarget code=dangling_edge subject=sequence:jobs.queue_id_seq message=edge sequence:jobs.queue_id_seq -[depends]-> schema:jobs references a fact not in the base
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\242-background-processing-release\supabase\migrations\20260817214621_declarative_sync.sql
...
grant delete, insert, maintain, references, select, trigger, truncate, update on table "jobs"."worker_outbox" to "postgres";
```
<!-- declarative-schema-command-result case="242-background-processing-release" engine="next" command="sync" status="ERROR" -->
...
```
<!-- declarative-schema-command-result case="242-background-processing-release" engine="next" command="sync-verification" status="ERROR" -->
...
CREATE SCHEMA jobs AUTHORIZATION postgres;
...
ALTER TABLE jobs.queue
  ADD CONSTRAINT queue_status_check CHECK (status = ANY (ARRAY['queued'::text, 'working'::text, 'done'::text]));
...
ALTER TABLE jobs.secret_refs
  ADD CONSTRAINT secret_refs_pkey PRIMARY KEY (name);
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\242-background-processing-release-legacy\supabase\migrations\20260817214747_242_background_processing_release_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\242-background-processing-release-legacy\supabase\.temp\pgdelta\debug\20260817-214747-apply-error
...
```text
The declarative baseline failed, so the background queue, retry, and dead-letter release evolution transition was skipped.
```
<!-- declarative-schema-command-result case="242-background-processing-release" engine="legacy" command="sync" status="ERROR" -->
