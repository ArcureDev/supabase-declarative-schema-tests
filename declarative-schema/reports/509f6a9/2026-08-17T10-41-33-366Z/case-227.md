# Case: 227-storage-object-policy-hardening
...
for insert
to authenticated
with check (bucket_id = 'transition-227');
```
...
### `20260817195543_227_storage_object_policy_hardening_baseline.sql`
...
- Raw sync result: **OK**
- Assertion: **ERROR**
- The generated migration did not satisfy its declared SQL-shape contract. No transition migration was generated. Missing operation(s): alter the supported Storage policy.
...
- Command: `npx supabase db schema declarative sync --apply --name 227_storage_object_policy_hardening_baseline --debug`
- Result: **OK**
- Duration: `39.9s`
...
Applying migration 20260817195543_227_storage_object_policy_hardening_baseline.sql...
Initialising schema...
+ ulimit -n
+ '[' -n '' ']'
+ export ERL_CRASH_DUMP=/tmp/erl_crash.dump
...
```
<!-- declarative-schema-command-result case="227-storage-object-policy-hardening" engine="next" command="sync" status="ERROR" -->
...
```
<!-- declarative-schema-command-result case="227-storage-object-policy-hardening" engine="next" command="sync-verification" status="ERROR" -->
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\227-storage-object-policy-hardening-legacy\supabase\migrations\20260817195730_227_storage_object_policy_hardening_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\227-storage-object-policy-hardening-legacy\supabase\.temp\pgdelta\debug\20260817-195730-apply-error
...
```text
The declarative baseline failed, so the preserve Storage policy and relation identity while restricting uploads to the caller folder transition was skipped.
```
<!-- declarative-schema-command-result case="227-storage-object-policy-hardening" engine="legacy" command="sync" status="ERROR" -->
