# Case: 240-rag-search-extension-release
...
### `20260817213824_240_rag_search_extension_release_baseline.sql`
...
create table "app"."documents" (
  ...
  "model_version"      text   not null default 'embed-v1'::text,
  constraint "documents_pkey" primary key (id),
  constraint "documents_storage_object_key_key" unique (storage_object_key)
);
...
alter table "app"."catalog_items"
  add column "search_document" tsvector generated always as (to_tsvector('simple'::regconfig, ((title || ' '::text) || description))) stored;

alter table "app"."chunks"
  add constraint "chunks_document_id_fkey" foreign key (document_id) references app.documents(id);

create index catalog_search_idx on app.catalog_items using gin (search_document);

create index catalog_title_trgm_idx on app.catalog_items using gin (title public.gin_trgm_ops);

create index chunks_embedding_v1_idx on app.chunks using hnsw (embedding extensions.vector_cosine_ops);
...
grant delete, insert, maintain, references, select, trigger, truncate, update on table "app"."catalog_items" to "postgres";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "app"."chunks" to "postgres";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "app"."documents" to "postgres";
...
- Assertion: **ERROR**
- The generated migration did not satisfy its declared SQL-shape contract. Raw sync status was WARNING. Missing operation(s): creates the application text-search configuration, creates the search API.

## Generated transition migration files

### `20260817213923_declarative_sync.sql`

```sql
set local check_function_bodies = off;

drop index "app"."catalog_title_trgm_idx";
...
alter table "app"."catalog_items"
  add column "catalog_search_v2" tsvector generated always as (to_tsvector('app.catalog_english'::regconfig, ((title || ' '::text) || description))) stored;

alter table "app"."chunks"
  add column "embedding_v2" extensions.vector(4);

alter table "app"."chunks"
  add column "model_version_v2" text not null default 'embed-v2'::text;

create or replace function app.search_catalog (
  query_text text
)
  returns table (
    item_id bigint,
...
alter table "app"."rag_jobs"
  add constraint "rag_jobs_document_id_fkey" foreign key (document_id) references app.documents(id);

create index catalog_facets_idx on app.catalog_items using gin (facets);

create index catalog_search_v2_idx on app.catalog_items using gin (catalog_search_v2);

create index catalog_title_trgm_idx on app.catalog_items using gin (title extensions.gin_trgm_ops);

create index chunks_embedding_v2_idx on app.chunks using hnsw (embedding_v2 extensions.vector_cosine_ops);

grant execute on function "app"."search_catalog"(text) to "postgres";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "app"."rag_jobs" to "postgres";
```
...
Applying migration 20260817213824_240_rag_search_extension_release_baseline.sql...
Initialising schema...
+ ulimit -n
+ '[' -n '' ']'
+ export ERL_CRASH_DUMP=/tmp/erl_crash.dump
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:app.rag_jobs_id_seq message=edge sequence:app.rag_jobs_id_seq -[depends]-> schema:app references a fact not in the base
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=default:app.catalog_items.catalog_search_v2 message=edge default:app.catalog_items.catalog_search_v2 -[depends]-> column:app.catalog_items.description references a fact not in the base
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=default:app.catalog_items.search_document message=edge default:app.catalog_items.search_document -[depends]-> column:app.catalog_items.description references a fact not in the base
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=default:app.catalog_items.catalog_search_v2 message=edge default:app.catalog_items.catalog_search_v2 -[depends]-> column:app.catalog_items.title references a fact not in the base
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=default:app.catalog_items.search_document message=edge default:app.catalog_items.search_document -[depends]-> column:app.catalog_items.title references a fact not in the base
...
pg-delta next diagnostic: origin=declarativeLoad code=unmodeled_kind message=1 unmodeled "text search configuration" object not managed by this engine (e.g. catalog_english) — v1 detects but does not model this kind; keep its DDL in _custom/ so re-exports preserve it and the shadow can elaborate dependents, and deliver it to targets via your migration channel
...
pg-delta next diagnostic: origin=declarativeTarget code=dangling_edge subject=default:app.catalog_items.search_document message=edge default:app.catalog_items.search_document -[depends]-> column:app.catalog_items.description references a fact not in the base
pg-delta next diagnostic: origin=declarativeTarget code=dangling_edge subject=default:app.catalog_items.search_document message=edge default:app.catalog_items.search_document -[depends]-> column:app.catalog_items.title references a fact not in the base
...
pg-delta next diagnostic: origin=declarativeDrift code=unmodeled_drift message=1 unmodeled "text search configuration" object exist in the desired state (shadow) but NOT on the target (app.catalog_english) — this engine models no facts for this kind, so no planned statement can create them, and any planned statement that depends on one will fail on the target. Deliver them through the same migration channel that delivers your _custom/ SQL, then re-plan
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\240-rag-search-extension-release\supabase\migrations\20260817213923_declarative_sync.sql
...
- Command: `npx supabase db schema declarative sync --apply --name 240_rag_search_extension_release_baseline --debug`
- Result: **OK**
- Duration: `59.4s`
...
grant delete, insert, maintain, references, select, trigger, truncate, update on table "app"."rag_jobs" to "postgres";
```
<!-- declarative-schema-command-result case="240-rag-search-extension-release" engine="next" command="sync" status="ERROR" -->
...
```
<!-- declarative-schema-command-result case="240-rag-search-extension-release" engine="next" command="sync-verification" status="ERROR" -->
...
CREATE TABLE app.catalog_items (
  ...
  search_document tsvector GENERATED ALWAYS AS (to_tsvector('simple'::regconfig, ((title || ' '::text) || description))) STORED
);
...
CREATE TABLE app.documents (
  id                 bigint NOT NULL,
  storage_object_key text   NOT NULL,
  model_version      text   DEFAULT 'embed-v1'::text NOT NULL
);
...
ALTER TABLE app.documents
  ADD CONSTRAINT documents_storage_object_key_key UNIQUE (storage_object_key);
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\240-rag-search-extension-release-legacy\supabase\migrations\20260817214037_240_rag_search_extension_release_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\240-rag-search-extension-release-legacy\supabase\.temp\pgdelta\debug\20260817-214037-apply-error
...
```text
The declarative baseline failed, so the AI/RAG, search catalog, and extension relocation evolution transition was skipped.
```
<!-- declarative-schema-command-result case="240-rag-search-extension-release" engine="legacy" command="sync" status="ERROR" -->
