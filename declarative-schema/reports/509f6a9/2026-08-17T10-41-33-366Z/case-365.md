# Case: 365-sequence-restart

- Scenario pack: `Schemas, tables, columns, and sequences catalogue scenarios` / `sequence-restart`
- Catalogue atoms: `PG-CAT-STC-09::sequence.restart`

## Baseline state A

```sql
-- Covers PG-CAT-STC-09::sequence.restart. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for sequence.restart must be refused, not silently omitted.
```
...
### `20260818080524_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\365-sequence-restart\supabase\migrations\20260818080524_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="365-sequence-restart" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="365-sequence-restart" engine="next" command="sync-verification" status="OK" -->
...
### `20260818080703_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\365-sequence-restart-legacy\supabase\migrations\20260818080703_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="365-sequence-restart" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="365-sequence-restart" engine="legacy" command="sync-verification" status="OK" -->
