# Case: 598-boundary-cluster-role

- Scenario pack: `Explicit PostgreSQL boundaries catalogue scenarios` / `boundary-cluster-role`
- Catalogue atoms: `PG-CAT-BND-04::boundary.cluster-role`

## Baseline state A

```sql
-- Covers PG-CAT-BND-04::boundary.cluster-role. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for boundary.cluster-role must be refused, not silently omitted.
```
...
### `20260818192106_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\598-boundary-cluster-role\supabase\migrations\20260818192106_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="598-boundary-cluster-role" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="598-boundary-cluster-role" engine="next" command="sync-verification" status="OK" -->
...
### `20260818192235_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\598-boundary-cluster-role-legacy\supabase\migrations\20260818192235_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="598-boundary-cluster-role" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="598-boundary-cluster-role" engine="legacy" command="sync-verification" status="OK" -->
