# Case: 434-rule-enable

- Scenario pack: `Constraints, indexes, statistics, and rules catalogue scenarios` / `rule-enable`
- Catalogue atoms: `PG-CAT-CIX-09::rule.enable`

## Baseline state A

```sql
-- Covers PG-CAT-CIX-09::rule.enable. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for rule.enable must be refused, not silently omitted.
```
...
### `20260818113448_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\434-rule-enable\supabase\migrations\20260818113448_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="434-rule-enable" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="434-rule-enable" engine="next" command="sync-verification" status="OK" -->
...
### `20260818113607_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\434-rule-enable-legacy\supabase\migrations\20260818113607_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="434-rule-enable" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="434-rule-enable" engine="legacy" command="sync-verification" status="OK" -->
