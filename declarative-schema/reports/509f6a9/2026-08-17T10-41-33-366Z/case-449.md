# Case: 449-partition-strategy

- Scenario pack: `Partitions and inheritance catalogue scenarios` / `partition-strategy`
- Catalogue atoms: `PG-CAT-PRT-03::partition.strategy`

## Baseline state A

```sql
-- Covers PG-CAT-PRT-03::partition.strategy. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for partition.strategy must be refused, not silently omitted.
```
...
### `20260818121653_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\449-partition-strategy\supabase\migrations\20260818121653_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="449-partition-strategy" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="449-partition-strategy" engine="next" command="sync-verification" status="OK" -->
...
### `20260818121812_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\449-partition-strategy-legacy\supabase\migrations\20260818121812_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="449-partition-strategy" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="449-partition-strategy" engine="legacy" command="sync-verification" status="OK" -->
