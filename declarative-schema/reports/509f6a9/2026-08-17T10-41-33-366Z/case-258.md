# Case: 258-realtime-publication-removal
...
alter publication supabase_realtime add table public.realtime_feed_258;
```
...
- Command: `npx supabase db schema declarative sync --apply --name 258_realtime_publication_removal_baseline --debug`
- Result: **ERROR**
- Duration: `40.6s`
- Exit code: `1`
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:public.realtime_feed_258_id_seq message=edge sequence:public.realtime_feed_258_id_seq -[depends]-> schema:public references a fact not in the base
...
```text
The declarative baseline failed, so the remove a populated table from Supabase Realtime without replacing it transition was skipped.
```
<!-- declarative-schema-command-result case="258-realtime-publication-removal" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.realtime_feed_258
  REPLICA IDENTITY FULL;

ALTER TABLE public.realtime_feed_258
  ADD CONSTRAINT realtime_feed_258_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_feed_258 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_feed_258 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_feed_258 TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\258-realtime-publication-removal-legacy\supabase\migrations\20260817232521_258_realtime_publication_removal_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\258-realtime-publication-removal-legacy\supabase\.temp\pgdelta\debug\20260817-232521-apply-error
...
The declarative baseline failed, so the remove a populated table from Supabase Realtime without replacing it transition was skipped.
```
<!-- declarative-schema-command-result case="258-realtime-publication-removal" engine="legacy" command="sync" status="ERROR" -->
