# Case: 533-role-password-redaction

- Scenario pack: `Roles, ownership, grants, and row-level security catalogue scenarios` / `role-password-redaction`
- Catalogue atoms: `PG-CAT-ROL-01::role.password-redaction`

## Baseline state A

```sql
-- Covers PG-CAT-ROL-01::role.password-redaction. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for role.password-redaction must be refused, not silently omitted.
```
...
### `20260818161549_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\533-role-password-redaction\supabase\migrations\20260818161549_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="533-role-password-redaction" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="533-role-password-redaction" engine="next" command="sync-verification" status="OK" -->
...
### `20260818161709_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\533-role-password-redaction-legacy\supabase\migrations\20260818161709_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="533-role-password-redaction" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="533-role-password-redaction" engine="legacy" command="sync-verification" status="OK" -->
