...
### `20260817165243_destructive_change_warning_baseline.sql`
...
- Raw sync result: **OK**
- Assertion: **WARNING**
- The command completed without satisfying the destructive-column-drop safety contract. No explicit destructive-change or data-loss diagnostic was present. The output identified public.destructive_change_guard. The output identified column doomed_value.

## Generated transition migration files

### `20260817165323_declarative_sync.sql`
...
Applying migration 20260817165243_destructive_change_warning_baseline.sql...
Initialising schema...
+ ulimit -n
+ '[' -n '' ']'
+ export ERL_CRASH_DUMP=/tmp/erl_crash.dump
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:public.destructive_change_guard_id_seq message=edge sequence:public.destructive_change_guard_id_seq -[depends]-> schema:public references a fact not in the base
...
pg-delta next diagnostic: origin=declarativeTarget code=dangling_edge subject=sequence:public.destructive_change_guard_id_seq message=edge sequence:public.destructive_change_guard_id_seq -[depends]-> schema:public references a fact not in the base
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\183-destructive-change-warning\supabase\migrations\20260817165323_declarative_sync.sql
...
alter table "public"."destructive_change_guard"
  drop column "doomed_value";
```
<!-- declarative-schema-command-result case="183-destructive-change-warning" engine="next" command="sync" status="WARNING" -->
...
ALTER TABLE public.destructive_change_guard
  ADD CONSTRAINT destructive_change_guard_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.destructive_change_guard TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.destructive_change_guard TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.destructive_change_guard TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\183-destructive-change-warning-legacy\supabase\migrations\20260817165430_destructive_change_warning_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\183-destructive-change-warning-legacy\supabase\.temp\pgdelta\debug\20260817-165430-apply-error
...
```
<!-- declarative-schema-command-result case="183-destructive-change-warning" engine="legacy" command="sync" status="ERROR" -->
