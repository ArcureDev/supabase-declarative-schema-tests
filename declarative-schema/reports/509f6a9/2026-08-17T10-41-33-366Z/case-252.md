# Case: 252-auth-users-trigger-hardening
...
- Command: `npx supabase db schema declarative sync --apply --name 252_auth_users_trigger_hardening_baseline --debug`
- Result: **ERROR**
- Duration: `40.1s`
- Exit code: `1`
...
```text
The declarative baseline failed, so the harden an auth.users profile trigger while preserving trigger identity, profile data, and cascade behavior transition was skipped.
```
<!-- declarative-schema-command-result case="252-auth-users-trigger-hardening" engine="next" command="sync" status="ERROR" -->
...
REVOKE ALL ON FUNCTION public.mirror_auth_user_252() FROM PUBLIC;

GRANT ALL ON FUNCTION public.mirror_auth_user_252() TO supabase_auth_admin;
...
ALTER TABLE public.auth_profile_252
  ADD CONSTRAINT auth_profile_252_pkey PRIMARY KEY (user_id);

ALTER TABLE public.auth_profile_252
  ADD CONSTRAINT auth_profile_252_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.auth_profile_252 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.auth_profile_252 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.auth_profile_252 TO service_role;
...
ALTER TABLE public.auth_trigger_snapshot_252
  ADD CONSTRAINT auth_trigger_snapshot_252_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.auth_trigger_snapshot_252 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.auth_trigger_snapshot_252 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.auth_trigger_snapshot_252 TO service_role;
...
ALTER TABLE public.transition_anchor_252
  ADD CONSTRAINT transition_anchor_252_pkey PRIMARY KEY (case_no);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_252 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_252 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_252 TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\252-auth-users-trigger-hardening-legacy\supabase\migrations\20260817221038_252_auth_users_trigger_hardening_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\252-auth-users-trigger-hardening-legacy\supabase\.temp\pgdelta\debug\20260817-221038-apply-error
...
The declarative baseline failed, so the harden an auth.users profile trigger while preserving trigger identity, profile data, and cascade behavior transition was skipped.
```
<!-- declarative-schema-command-result case="252-auth-users-trigger-hardening" engine="legacy" command="sync" status="ERROR" -->
