...
-- Anonymous Auth users also assume authenticated; the claim separates them.
create policy anonymous_owned_drafts_250
on public.anonymous_drafts_250
for select
to authenticated
using (
  owner_id = (select auth.uid())
  and coalesce((select auth.jwt() ->> 'is_anonymous'), 'false')::boolean = true
);
...
```sql
insert into public.transition_anchor_250 (case_no, payload)
values (250, 'anonymous-rls');

insert into public.anonymous_drafts_250 (owner_id, body)
values
  ('25000000-0000-0000-0000-000000000001', 'anonymous draft'),
  ('25000000-0000-0000-0000-000000000002', 'registered draft');
```
...
- Command: `npx supabase db schema declarative sync --apply --name 250_anonymous_rls_baseline --debug`
- Result: **ERROR**
- Duration: `39.9s`
- Exit code: `1`
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:public.anonymous_drafts_250_id_seq message=edge sequence:public.anonymous_drafts_250_id_seq -[depends]-> schema:public references a fact not in the base
...
```text
The declarative baseline failed, so the add an explicit anonymous-user policy without weakening registered-user isolation transition was skipped.
```
<!-- declarative-schema-command-result case="250-anonymous-rls" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.anonymous_drafts_250
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.anonymous_drafts_250
  ADD CONSTRAINT anonymous_drafts_250_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.anonymous_drafts_250 TO anon;

GRANT MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE ON public.anonymous_drafts_250 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.anonymous_drafts_250 TO service_role;

CREATE POLICY registered_owned_drafts_250 ON public.anonymous_drafts_250
  FOR SELECT
  TO authenticated
  USING (((owner_id = ( SELECT auth.uid() AS uid)) AND ((COALESCE(( SELECT (auth.jwt() ->> 'is_anonymous'::text)), 'false'::text))::boolean = false)));
...
ALTER TABLE public.rls_probe_250
  ADD CONSTRAINT rls_probe_250_id_check CHECK (id);

ALTER TABLE public.rls_probe_250
  ADD CONSTRAINT rls_probe_250_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.rls_probe_250 TO anon;

GRANT INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE ON public.rls_probe_250 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.rls_probe_250 TO service_role;
...
ALTER TABLE public.transition_anchor_250
  ADD CONSTRAINT transition_anchor_250_pkey PRIMARY KEY (case_no);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_250 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_250 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_250 TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\250-anonymous-rls-legacy\supabase\migrations\20260817220640_250_anonymous_rls_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\250-anonymous-rls-legacy\supabase\.temp\pgdelta\debug\20260817-220640-apply-error
...
The declarative baseline failed, so the add an explicit anonymous-user policy without weakening registered-user isolation transition was skipped.
```
<!-- declarative-schema-command-result case="250-anonymous-rls" engine="legacy" command="sync" status="ERROR" -->
