# Case: 259-filtered-column-publication
...
- Command: `npx supabase db schema declarative sync --apply --name 259_filtered_column_publication_baseline --debug`
- Result: **ERROR**
- Duration: `40.6s`
- Exit code: `1`
...
```text
The declarative baseline failed, so the narrow a Realtime publication membership to selected columns and rows transition was skipped.
```
<!-- declarative-schema-command-result case="259-filtered-column-publication" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.realtime_filtered_259
  ADD CONSTRAINT realtime_filtered_259_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_filtered_259 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_filtered_259 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.realtime_filtered_259 TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\259-filtered-column-publication-legacy\supabase\migrations\20260817232757_259_filtered_column_publication_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\259-filtered-column-publication-legacy\supabase\.temp\pgdelta\debug\20260817-232758-apply-error
...
The declarative baseline failed, so the narrow a Realtime publication membership to selected columns and rows transition was skipped.
```
<!-- declarative-schema-command-result case="259-filtered-column-publication" engine="legacy" command="sync" status="ERROR" -->
