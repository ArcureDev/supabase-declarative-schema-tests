...
- Scenario pack: `Types, domains, ranges, and casts catalogue scenarios` / `cast-drop`
- Catalogue atoms: `PG-CAT-TYP-06::cast.drop`

## Baseline state A

```sql
-- Covers PG-CAT-TYP-06::cast.drop. Keep public.transition_anchor identity stable. This atom is an explicit supported/unsupported boundary, not an accidental omission.
...
-- Desired change for cast.drop must be refused, not silently omitted.
```
...
### `20260818134700_declarative_sync.sql`
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\481-cast-drop\supabase\migrations\20260818134700_declarative_sync.sql
...
grant maintain, references, trigger, truncate on table "public"."transition_anchor" to "service_role";
```
<!-- declarative-schema-command-result case="481-cast-drop" engine="next" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="481-cast-drop" engine="next" command="sync-verification" status="OK" -->
...
### `20260818134819_declarative_sync.sql`
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\481-cast-drop-legacy\supabase\migrations\20260818134819_declarative_sync.sql
...
GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor TO service_role;
```
<!-- declarative-schema-command-result case="481-cast-drop" engine="legacy" command="sync" status="ERROR" -->
...
- Result: **OK**
- Duration: `0.1s`
<!-- declarative-schema-command-result case="481-cast-drop" engine="legacy" command="sync-verification" status="OK" -->
