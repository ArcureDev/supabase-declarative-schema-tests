# Case: 255-storage-policy-matrix
...
- Command: `npx supabase db schema declarative sync --apply --name 255_storage_policy_matrix_baseline --debug`
- Result: **ERROR**
- Duration: `49.3s`
- Exit code: `1`
...
```text
The declarative baseline failed, so the harden the complete Storage object policy command matrix in place transition was skipped.
```
<!-- declarative-schema-command-result case="255-storage-policy-matrix" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.storage_policy_identity_255
  ADD CONSTRAINT storage_policy_identity_255_pkey PRIMARY KEY (policy_name);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.storage_policy_identity_255 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.storage_policy_identity_255 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.storage_policy_identity_255 TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\255-storage-policy-matrix-legacy\supabase\migrations\20260817221805_255_storage_policy_matrix_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\255-storage-policy-matrix-legacy\supabase\.temp\pgdelta\debug\20260817-221805-apply-error
...
The declarative baseline failed, so the harden the complete Storage object policy command matrix in place transition was skipped.
```
<!-- declarative-schema-command-result case="255-storage-policy-matrix" engine="legacy" command="sync" status="ERROR" -->
