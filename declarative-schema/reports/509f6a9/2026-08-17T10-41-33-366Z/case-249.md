# Case: 249-jwt-custom-claims-mfa-rls
...
- Command: `npx supabase db schema declarative sync --apply --name 249_jwt_custom_claims_mfa_rls_baseline --debug`
- Result: **ERROR**
- Duration: `40.5s`
- Exit code: `1`
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:public.protected_records_249_id_seq message=edge sequence:public.protected_records_249_id_seq -[depends]-> schema:public references a fact not in the base
...
```text
The declarative baseline failed, so the require tenant custom claims, auth.uid ownership, and AAL2 for protected records transition was skipped.
```
<!-- declarative-schema-command-result case="249-jwt-custom-claims-mfa-rls" engine="next" command="sync" status="ERROR" -->
...
ALTER TABLE public.protected_records_249
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.protected_records_249
  ADD CONSTRAINT protected_records_249_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.protected_records_249 TO anon;

GRANT MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE ON public.protected_records_249 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.protected_records_249 TO service_role;

CREATE POLICY jwt_tenant_records_249 ON public.protected_records_249
  FOR SELECT
  TO authenticated
  USING (((owner_id = ( SELECT auth.uid() AS uid)) AND (tenant_id = ( SELECT ((auth.jwt() -> 'app_metadata'::text) ->> 'tenant_id'::text)))));
...
ALTER TABLE public.rls_probe_249
  ADD CONSTRAINT rls_probe_249_pkey PRIMARY KEY (label);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.rls_probe_249 TO anon;

GRANT INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE ON public.rls_probe_249 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.rls_probe_249 TO service_role;
...
ALTER TABLE public.transition_anchor_249
  ADD CONSTRAINT transition_anchor_249_pkey PRIMARY KEY (case_no);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_249 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_249 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_249 TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\249-jwt-custom-claims-mfa-rls-legacy\supabase\migrations\20260817220406_249_jwt_custom_claims_mfa_rls_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\249-jwt-custom-claims-mfa-rls-legacy\supabase\.temp\pgdelta\debug\20260817-220406-apply-error
...
The declarative baseline failed, so the require tenant custom claims, auth.uid ownership, and AAL2 for protected records transition was skipped.
```
<!-- declarative-schema-command-result case="249-jwt-custom-claims-mfa-rls" engine="legacy" command="sync" status="ERROR" -->
