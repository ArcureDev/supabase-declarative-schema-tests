# Case: 253-supabase-role-boundaries
...
- Command: `npx supabase db schema declarative sync --apply --name 253_supabase_role_boundaries_baseline --debug`
- Result: **ERROR**
- Duration: `49.9s`
- Exit code: `1`
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:public.role_boundary_253_id_seq message=edge sequence:public.role_boundary_253_id_seq -[depends]-> schema:public references a fact not in the base
...
```text
The declarative baseline failed, so the tighten application ACLs across anon, authenticated, service_role, and supabase_auth_admin transition was skipped.
```
<!-- declarative-schema-command-result case="253-supabase-role-boundaries" engine="next" command="sync" status="ERROR" -->
...
GRANT ALL ON FUNCTION public.role_admin_marker_253() TO authenticated;
...
ALTER TABLE public.role_boundary_253
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.role_boundary_253
  ADD CONSTRAINT role_boundary_253_pkey PRIMARY KEY (id);

ALTER TABLE public.role_boundary_253
  ADD CONSTRAINT role_boundary_253_visibility_check CHECK (visibility = ANY (ARRAY['public'::text, 'private'::text]));

GRANT MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE ON public.role_boundary_253 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.role_boundary_253 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.role_boundary_253 TO service_role;

CREATE POLICY authenticated_public_rows_253 ON public.role_boundary_253
  FOR SELECT
  TO authenticated
...
ALTER TABLE public.transition_anchor_253
  ADD CONSTRAINT transition_anchor_253_pkey PRIMARY KEY (case_no);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_253 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_253 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.transition_anchor_253 TO service_role;
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\253-supabase-role-boundaries-legacy\supabase\migrations\20260817221341_253_supabase_role_boundaries_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\253-supabase-role-boundaries-legacy\supabase\.temp\pgdelta\debug\20260817-221341-apply-error
...
The declarative baseline failed, so the tighten application ACLs across anon, authenticated, service_role, and supabase_auth_admin transition was skipped.
```
<!-- declarative-schema-command-result case="253-supabase-role-boundaries" engine="legacy" command="sync" status="ERROR" -->
