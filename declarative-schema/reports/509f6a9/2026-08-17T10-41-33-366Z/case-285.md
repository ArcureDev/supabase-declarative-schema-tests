# Case: 285-multi-file-declarative-ordering

- Coverage: Resolve dependencies across deliberately reverse-named declarative files and apply them in catalog-safe order.
- Requirements: `C2`
...
## Reset multi-file fixture (config plane)

- Command: `npx supabase db reset --local --no-seed --debug`
- Result: **OK**
- Duration: `32.3s`

## Apply ordered multi-file declaration (config plane)
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:public.coverage_order_items_id_seq message=edge sequence:public.coverage_order_items_id_seq -[depends]-> schema:public references a fact not in the base
...
## Assert cross-file dependencies (config plane)
...
```text
Blocked by unsuccessful phase(s): sync.
```
...
(no output)
```
<!-- declarative-schema-command-result case="285-multi-file-declarative-ordering" engine="next" command="generate" status="ERROR" -->
<!-- declarative-schema-command-result case="285-multi-file-declarative-ordering" engine="next" command="sync" status="ERROR" -->
<!-- declarative-schema-command-result case="285-multi-file-declarative-ordering" engine="next" command="sync-verification" status="ERROR" -->
