# Case: 237-tenant-modular-graphql-release
...
### `20260817212902_237_tenant_modular_graphql_release_baseline.sql`
...
create table "tenant"."memberships" (
  ...
  constraint "memberships_member_role_check" check ((member_role = ANY (ARRAY['owner'::text, 'member'::text]))),
  constraint "memberships_pkey" primary key (organization_id, user_id)
);

alter table "tenant"."memberships"
...
create or replace function tenant.custom_access_token_hook (
  event jsonb
)
  returns jsonb
  language sql
  stable
  security definer
  set search_path to 'pg_catalog', 'tenant'
  AS $function$
  select jsonb_set(
    event,
    '{claims}',
...
alter table "tenant"."memberships"
  add constraint "memberships_organization_id_fkey" foreign key (organization_id) references tenant.organizations(id);

create view "api"."organization_summary" with (security_invoker=true) AS  SELECT id,
    name
   FROM tenant.organizations;

create policy "memberships_self_read" on "tenant"."memberships"
  for select
  to "authenticated"
  using ((user_id = (NULLIF(current_setting('request.jwt.claim.sub'::text, true), ''::text))::uuid));

create policy "organizations_member_read" on "tenant"."organizations"
  for select
  to "authenticated"
  using ((exists ( select 1
   from tenant.memberships membership
  where ((membership.organization_id = organizations.id) AND (membership.user_id = (NULLIF(current_setting('request.jwt.claim.sub'::text, true), ''::text))::uuid)))));

comment on view "api"."organization_summary" is '@graphql({"name":"Organization"})';

revoke all on function "tenant"."custom_access_token_hook"(jsonb) from public;

grant execute on function "tenant"."custom_access_token_hook"(jsonb) to "postgres", "supabase_auth_admin";
...
grant create, usage on schema "tenant" to "postgres";
...
grant select on table "tenant"."memberships" to "authenticated";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "tenant"."memberships" to "postgres";

grant select on table "tenant"."organizations" to "authenticated";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "tenant"."organizations" to "postgres";

grant select on table "api"."organization_summary" to "authenticated";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "api"."organization_summary" to "postgres";
```
...
- Raw sync result: **OK**
- Assertion: **ERROR**
- The generated migration did not satisfy its declared SQL-shape contract. Missing operation(s): grants invitation management privileges. Forbidden operation(s): drops an API compatibility object.

## Generated transition migration files

### `20260817212943_declarative_sync.sql`

```sql
set local check_function_bodies = off;

drop view "api"."organization_summary";

create table "tenant"."invitations" (
  "id"              bigint                   generated always as identity not null,
  "organization_id" uuid                     not null,
  "email"           text                     not null,
  "invited_role"    text                     not null default 'member'::text,
  "expires_at"      timestamp with time zone not null,
  "accepted_at"     timestamp with time zone,
  constraint "invitations_organization_id_email_key" unique (organization_id, email),
...
alter table "tenant"."memberships"
  add column "billing_role" text not null default 'collaborator'::text;
...
alter table "tenant"."invitations"
  add constraint "invitations_organization_id_fkey" foreign key (organization_id) references tenant.organizations(id);

alter table "tenant"."memberships"
  add constraint "memberships_billing_role_check" check ((billing_role = ANY (ARRAY['admin'::text, 'collaborator'::text, 'viewer'::text])));

create view "api"."organization_summary" with (security_invoker=true) AS  SELECT organization.id,
    organization.name,
    count(membership.user_id) AS member_count
   FROM (tenant.organizations organization
     LEFT JOIN tenant.memberships membership ON ((membership.organization_id = organization.id)))
  GROUP BY organization.id, organization.name;

create index invitations_pending_idx on tenant.invitations using btree (organization_id, expires_at)
  where (accepted_at is null);

create policy "invitations_owner_manage" on "tenant"."invitations"
  for all
  to "authenticated"
  using ((exists ( select 1
   from tenant.memberships membership
  where
    ((membership.organization_id = invitations.organization_id) AND (membership.member_role = 'owner'::text) AND (membership.user_id =
    (NULLIF(current_setting('request.jwt.claim.sub'::text, true), ''::text))::uuid)))))
...
comment on view "api"."organization_summary" is '@graphql({"name":"Organization","description":"Tenant API v2"})';

grant delete, insert, select, update on table "tenant"."invitations" to "authenticated";

grant delete, insert, maintain, references, select, trigger, truncate, update on table "tenant"."invitations" to "postgres";
...
- Command: `npx supabase db schema declarative sync --apply --name 237_tenant_modular_graphql_release_baseline --debug`
- Result: **OK**
- Duration: `59.2s`
...
Applying migration 20260817212902_237_tenant_modular_graphql_release_baseline.sql...
Initialising schema...
+ ulimit -n
+ '[' -n '' ']'
+ export ERL_CRASH_DUMP=/tmp/erl_crash.dump
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:tenant.invitations_id_seq message=edge sequence:tenant.invitations_id_seq -[depends]-> schema:tenant references a fact not in the base
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\237-tenant-modular-graphql-release\supabase\migrations\20260817212943_declarative_sync.sql
...
grant delete, insert, maintain, references, select, trigger, truncate, update on table "api"."organization_summary" to "postgres";
```
<!-- declarative-schema-command-result case="237-tenant-modular-graphql-release" engine="next" command="sync" status="ERROR" -->
...
```
<!-- declarative-schema-command-result case="237-tenant-modular-graphql-release" engine="next" command="sync-verification" status="ERROR" -->
...
Applied 19 statements in 1 round(s).
Generated migration SQL:
...
CREATE SCHEMA tenant AUTHORIZATION postgres;

GRANT USAGE ON SCHEMA tenant TO authenticated;

GRANT USAGE ON SCHEMA tenant TO supabase_auth_admin;

CREATE FUNCTION tenant.custom_access_token_hook (
  event jsonb
)
  RETURNS jsonb
  LANGUAGE sql
...
GRANT ALL ON FUNCTION tenant.custom_access_token_hook(jsonb) TO supabase_auth_admin;
...
ALTER TABLE tenant.memberships
  ADD CONSTRAINT memberships_member_role_check CHECK (member_role = ANY (ARRAY['owner'::text, 'member'::text]));

ALTER TABLE tenant.memberships
  ADD CONSTRAINT memberships_pkey PRIMARY KEY (organization_id, user_id);

GRANT SELECT ON tenant.memberships TO authenticated;
...
GRANT SELECT ON api.organization_summary TO authenticated;
...
GRANT SELECT ON tenant.organizations TO authenticated;
...
CREATE POLICY organizations_member_read ON tenant.organizations
  ...
  USING ((EXISTS ( SELECT 1
   FROM tenant.memberships membership
  WHERE ((membership.organization_id = organizations.id) AND (membership.user_id = (NULLIF(current_setting('request.jwt.claim.sub'::text, true), ''::text))::uuid)))));
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\237-tenant-modular-graphql-release-legacy\supabase\migrations\20260817213110_237_tenant_modular_graphql_release_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\237-tenant-modular-graphql-release-legacy\supabase\.temp\pgdelta\debug\20260817-213110-apply-error
...
```text
The declarative baseline failed, so the tenant, modular API, and GraphQL release evolution transition was skipped.
```
<!-- declarative-schema-command-result case="237-tenant-modular-graphql-release" engine="legacy" command="sync" status="ERROR" -->
