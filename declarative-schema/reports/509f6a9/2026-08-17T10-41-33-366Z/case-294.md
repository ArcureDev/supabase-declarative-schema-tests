# Case: 294-pg-delta-feature-flag

- Coverage: Apply the same declaration with pg-delta next and legacy in isolated reset states, proving the phase-level engine override.
- Requirements: `C5`
...
## Reset pg-delta flag fixture (config plane)

- Command: `npx supabase db reset --local --no-seed --debug`
- Result: **OK**
- Duration: `32.8s`

## Apply declaration with pg-delta next (config plane)

- Command: `npx supabase db schema declarative sync --apply --debug`
- Result: **ERROR**
- Duration: `39.9s`
- Exit code: `1`
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=default:public.coverage_pgdelta_flag.normalized message=edge default:public.coverage_pgdelta_flag.normalized -[depends]-> column:public.coverage_pgdelta_flag.source references a fact not in the base
...
pg-delta next diagnostic: origin=declarativeLoad code=dangling_edge subject=sequence:public.coverage_pgdelta_flag_id_seq message=edge sequence:public.coverage_pgdelta_flag_id_seq -[depends]-> schema:public references a fact not in the base
...
## Assert pg-delta next result (config plane)
...
```text
Blocked by unsuccessful phase(s): next-sync.
```

## Remove next-engine migration before legacy comparison (config plane)

- Command: `phase:clear-next-migration`
- Result: **SKIPPED**
- Duration: `0.0s`

```text
Blocked by unsuccessful phase(s): verify.
```

## Reset before the legacy comparison (config plane)

- Command: `phase:legacy-reset`
- Result: **SKIPPED**
- Duration: `0.0s`

```text
Blocked by unsuccessful phase(s): clear-next-migration.
```

## Apply declaration with pg-delta legacy (config plane)

- Command: `phase:legacy-sync`
- Result: **SKIPPED**
- Duration: `0.0s`

```text
Blocked by unsuccessful phase(s): legacy-reset.
```

## Assert pg-delta legacy result (config plane)

- Command: `phase:verify-legacy`
- Result: **SKIPPED**
- Duration: `0.0s`

```text
Blocked by unsuccessful phase(s): legacy-sync.
```
...
(no output)
```
<!-- declarative-schema-command-result case="294-pg-delta-feature-flag" engine="next" command="generate" status="ERROR" -->
<!-- declarative-schema-command-result case="294-pg-delta-feature-flag" engine="next" command="sync" status="ERROR" -->
<!-- declarative-schema-command-result case="294-pg-delta-feature-flag" engine="next" command="sync-verification" status="ERROR" -->
