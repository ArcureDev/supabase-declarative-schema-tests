# Case: 256-storage-reference-helpers
...
- Command: `npx supabase db schema declarative sync --apply --name 256_storage_reference_helpers_baseline --debug`
- Result: **ERROR**
- Duration: `59.3s`
- Exit code: `1`
...
```text
The declarative baseline failed, so the evolve an ACL-safe wrapper around Storage path reference helpers transition was skipped.
```
<!-- declarative-schema-command-result case="256-storage-reference-helpers" engine="next" command="sync" status="ERROR" -->
...
CREATE FUNCTION public.storage_path_facts_256 (
  path text
)
  RETURNS jsonb
  LANGUAGE sql
...
REVOKE ALL ON FUNCTION public.storage_path_facts_256(text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.storage_path_facts_256(text) TO authenticated;
...
ALTER TABLE public.storage_helper_probe_256
  ADD CONSTRAINT storage_helper_probe_256_pkey PRIMARY KEY (id);

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.storage_helper_probe_256 TO anon;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.storage_helper_probe_256 TO authenticated;

GRANT MAINTAIN, REFERENCES, TRIGGER, TRUNCATE ON public.storage_helper_probe_256 TO service_role;
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\256-storage-reference-helpers-legacy\supabase\migrations\20260817222108_256_storage_reference_helpers_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\256-storage-reference-helpers-legacy\supabase\.temp\pgdelta\debug\20260817-222108-apply-error
...
The declarative baseline failed, so the evolve an ACL-safe wrapper around Storage path reference helpers transition was skipped.
```
<!-- declarative-schema-command-result case="256-storage-reference-helpers" engine="legacy" command="sync" status="ERROR" -->
