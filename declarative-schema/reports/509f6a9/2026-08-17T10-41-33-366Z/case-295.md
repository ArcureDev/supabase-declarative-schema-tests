# Case: 295-interrupted-command-recovery

- Coverage: Interrupt a real declarative sync during its apply transaction, remove the fixture delay, retry, and assert complete recovery.
- Requirements: `C5`
...
## Restore simulated interrupted checkpoint (config plane)

- Command: `npx supabase db reset --local --no-seed --debug`
- Result: **OK**
- Duration: `32.6s`

## Interrupt declarative sync during apply (config plane)

- Command: `node --experimental-strip-types E:\workspace\supabase-declarative-schema-tests\declarative-schema\coverage\config\295-interrupted-command-recovery\interrupt-sync.mts`
- Result: **OK**
- Duration: `5.1s`

## Remove the fixture-only DDL delay (config plane)

- Command: `docker exec --interactive supabase_db_ds-shared-runtime psql --username postgres --dbname postgres --no-psqlrc --tuples-only --no-align --set ON_ERROR_STOP=1 --file -`
- Result: **OK**
- Duration: `0.1s`

## Converge partial checkpoint (config plane)
...
Applying migration 20260101000000_partial_checkpoint.sql...
Initialising schema...
+ ulimit -n
+ '[' -n '' ']'
+ export ERL_CRASH_DUMP=/tmp/erl_crash.dump
...
## Assert recovered complete state (config plane)
...
```text
Blocked by unsuccessful phase(s): recover.
```
...
(no output)
```
<!-- declarative-schema-command-result case="295-interrupted-command-recovery" engine="next" command="generate" status="ERROR" -->
<!-- declarative-schema-command-result case="295-interrupted-command-recovery" engine="next" command="sync" status="ERROR" -->
<!-- declarative-schema-command-result case="295-interrupted-command-recovery" engine="next" command="sync-verification" status="ERROR" -->
