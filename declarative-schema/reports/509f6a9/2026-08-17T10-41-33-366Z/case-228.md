# Case: 228-realtime-message-policy-hardening
...
for select
to authenticated
using ((select realtime.topic()) = 'transition:public');
```
...
### `20260817195855_228_realtime_message_policy_hardening_baseline.sql`
...
- Raw sync result: **OK**
- Assertion: **ERROR**
- The generated migration did not satisfy its declared SQL-shape contract. No transition migration was generated. Missing operation(s): alter the supported Realtime policy.
...
- Command: `npx supabase db schema declarative sync --apply --name 228_realtime_message_policy_hardening_baseline --debug`
- Result: **OK**
- Duration: `60.8s`
...
Applying migration 20260817195855_228_realtime_message_policy_hardening_baseline.sql...
Initialising schema...
+ ulimit -n
+ '[' -n '' ']'
+ export ERL_CRASH_DUMP=/tmp/erl_crash.dump
...
```
<!-- declarative-schema-command-result case="228-realtime-message-policy-hardening" engine="next" command="sync" status="ERROR" -->
...
```
<!-- declarative-schema-command-result case="228-realtime-message-policy-hardening" engine="next" command="sync-verification" status="ERROR" -->
...
Created new migration at E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\228-realtime-message-policy-hardening-legacy\supabase\migrations\20260817200042_228_realtime_message_policy_hardening_baseline.sql
...
Debug information saved to E:\workspace\supabase-declarative-schema-tests\declarative-schema\.tmp\run-bCM1EI\228-realtime-message-policy-hardening-legacy\supabase\.temp\pgdelta\debug\20260817-200042-apply-error
...
```text
The declarative baseline failed, so the preserve Realtime policy and relation identity while scoping private channels to auth.uid transition was skipped.
```
<!-- declarative-schema-command-result case="228-realtime-message-policy-hardening" engine="legacy" command="sync" status="ERROR" -->
